import type { Module, RichEvent, Selection } from '../types';
...
export function Inspector({ selected, setSelected, modules, richEvents }: InspectorProps) {
...
function EventSection({ title, icon, events, onPick }: EventSectionProps) {
  return (
      ...
      {events.length === 0 ? (
        ...
        >
          <span style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0, overflow: 'hidden' }}>
            <span style={{ width: 6, height: 6, borderRadius: '50%', background: e.incomplete ? 'var(--eventus-danger)' : 'var(--eventus-accent)', flexShrink: 0 }} />
