...
import { splitId } from '../../hooks/useAggregatedTopology';
...
export function ImpactView({ modules, richEvents, backendUrls }: ImpactViewProps) {
  ...
  useEffect(() => {
    ...
    // Resolve backend URL from namespaced ID (e.g. "backendId::moduleId")
    const parsed = splitId(sel.id);
    const isCrossService = parsed?.backendId === 'xsvc';
    if (isCrossService) {
      setError('Cross-service events span multiple backends — select a specific module for impact analysis.');
      setLoading(false);
      return;
    }
    const url = parsed && backendUrls ? (backendUrls.get(parsed.backendId) ?? '') : '';
    const originalId = parsed?.originalId ?? sel.id;

    setLoading(true);
    setError(null);
    const p = sel.kind === 'module'
      ? fetchModuleImpact(originalId, url).then(r => { setModuleResult(r); setEventResult(null); })
      : fetchEventImpact(originalId, url).then(r => { setEventResult(r); setModuleResult(null); });
